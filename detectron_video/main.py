...
while cap.isOpened():
    ...
    if count > 20:
        print("Processed 20 frames, stopping for demo purposes.")
        break
    ...
    for track in updated_sort:
        ...
        for i, box in enumerate(prediction_box_tensor):
            ...
            distance = np.linalg.norm(
                np.array(tracked_center) - np.array(prediction_box_center)
            )
        ...
        croped_image = full_image.crop((int(x1), int(y1), int(x2), int(y2)))
        os.makedirs(f"frames/croped/{track_id}", exist_ok=True)
        croped_image.save(f"frames/croped/{track_id}/frame_{count}_id_{track_id}.jpg")
    ...
    os.makedirs("frames/mask", exist_ok=True)
    # cv2.imwrite(f"frames/mask/masked_frame_{count}.jpg", mask)
    # cv2.imwrite(f"frames/mask/masked_result_frame_{count}.jpg", masked_result)
...
with open("trajectory.txt", "w") as f:
    for track_id in sorted(trajectories):
        ...
        for frame_idx, x1, y1, x2, y2 in trajectories[track_id]:
            f.write(
                f"Frame Index{frame_idx} X1:{x1:.2f} Y2:{y1:.2f} X2:{x2:.2f} Y2:{y2:.2f}\n"
            )
        f.write("\n")
